...
def aylik_yorum_serisi(df: pd.DataFrame, lookback_months: int = 24) -> pd.Series:
    """review_datetime_utc alanından ay bazında yorum adetlerini döndürür (son lookback_months)."""
...
def yorum_hizi_tahmini(df: pd.DataFrame, months_ahead: int = 12, base_window: int = 3, season_lookback_months: int = 24) -> pd.DataFrame:
    """Gelecek months_ahead ay için beklenen yorum adetlerini tahmin eder."""
...
def kac_ayda_yorum_hedefi(pred_df: pd.DataFrame, hedef_yorum_sayisi: int) -> Tuple[int, pd.DataFrame]:
    """Tahmin edilen aylık adetlerle hedef_yorum_sayisi'na kaç ayda ulaşılır?"""
...
def google_agirlik_hesapla(gun_farki: float) -> float:
    """
    Google Maps sıralama algoritmasına yakın parçalı ağırlık.
    Son 3 ay   → 1.00 (tam ağırlık)
    Son 6 ay   → 0.80
    Son 12 ay  → 0.50
    2-3 yıl    → 0.20
    4-5 yıl    → 0.05
...
def gereken_yeni_ortalama_google(
    ...
    """
    Google Maps sıralama + puan algoritmasına yakın tahmin:
    - Parçalı recency ağırlığı (3ay/6ay/12ay/2-3yıl/4-5yıl)
    - Bayesian average (az yorumlu işletmeleri dengeler)
    - Yeni yorumlar bugün yazılmış gibi → ağırlık = 1.0
    """
    if df is None or df.empty or "review_datetime_utc" not in df.columns or "review_rating" not in df.columns:
        return None
    try:
        ...
        # Mevcut ağırlıklı toplam + Bayes prior
        ...
        # Yeni yorumlar bugün yazılmış → ağırlık = 1.0
        W_yeni = float(yeni_yorum_sayisi) * 1.0

        if W_yeni <= 0:
            return None

        # (WR_mevcut + W_yeni * x) / (W_mevcut + W_yeni) = hedef_puan
...
def gereken_yeni_ortalama_basit(
    ...
    """
    Basit ortalama modeli:
    (mevcut_toplam_puan + yeni_yorum_sayisi * x) / (mevcut_yorum_sayisi + yeni_yorum_sayisi) = hedef_puan
    """
    try:
...
def agirlikli_puan_hesapla(df: pd.DataFrame, yari_omur_gun: int = 180) -> Tuple[float, float]:
    """
    Google benzeri zaman ağırlıklı ortalama puan.

    Yöntem: Üstel azalma (exponential decay)
      - Bugün yazılan yorum  → ağırlık = 1.0
      - yari_omur_gun önce   → ağırlık = 0.5
      - 2 × yari_omur önce   → ağırlık = 0.25

    Döner: (agirlikli_ortalama, efektif_yorum_sayisi)
    """
    df2 = df.copy()
...
if df is not None and not df.empty:
    ...
    # ==========================================
    # 🎯 GELECEK PERFORMANS HEDEFİ (YORUM HIZI + RECENCY)
    # ==========================================
    ...
    with col_sim_1:
        ...
        # Slider başlangıcı ekranda görülen anlık puana göre ayarlanır
    ...
    with col_sim_2:
        # 1) Bu kadar yorum kaç ayda gelir? (mevsimsellik + son dönem hız)
        ...
        if ay_sayisi <= 0 or used_months.empty:
            ...
            # 2) KPI ile aynı taban: Google ortalama + Google toplam yorum
