...
with tripadvisor_tab:
    st.markdown("### TripAdvisor Yorum Analizi")
    st.caption("TripAdvisor'dan indirdiğiniz Excel dosyasını yükleyin — veritabanına kaydedilir ve kalıcı olur.")

    ta_db_adet = db_tripadvisor_adet()
    if ta_db_adet > 0:
        st.success(f"✅ Veritabanında kayıtlı TripAdvisor yorumu: {ta_db_adet}")
    else:
        st.info("Henüz TripAdvisor verisi yok. İlk dosyayı yükleyin.")
    ...
    if ta_file is not None:
        ...
        if not ta_df_raw.empty:
            ...
            st.success(f"✅ {eklenen} yeni TripAdvisor yorumu veritabanına kaydedildi.")
            st.rerun()
    ...
    if ta_df is not None and not ta_df.empty:
        ...
        st.info("📂 Lütfen bir TripAdvisor Excel dosyası yükleyin.")
